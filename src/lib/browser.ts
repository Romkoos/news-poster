 ...
 */
export async function stopAndFlush(ctx: BrowserContext, traceOn: boolean) {
  try {
    if (traceOn) {
      const traceDir = path.resolve('debug-artifacts', 'trace');
      await ensureDirs([traceDir]);
      const out = path.resolve(traceDir, `trace-${Date.now()}.zip`);
      await ctx.tracing.stop({ path: out });
      log('Trace saved:', out);
      log('Open trace with: npx playwright show-trace "%s"', out);
    }
  } catch (e) {
    log('trace stop error:', e);
  }
  await ctx.close();
}
