...
export async function extractImageUrl(
  ...
  try {
    await (mediaContainer as any).evaluate((el: Element) => (el as HTMLElement).scrollIntoView({ block: 'center', inline: 'center' }));
...
function preferAkamai(u: string) {
  return /makostorepdl-a\.akamaihd\.net/i.test(u);
}
