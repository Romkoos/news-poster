...
// Модуль импортируется самым первым в src/index.ts, чтобы успеть выставить переменные окружения
// ДО загрузки onnxruntime-node/onnxruntime-web (иначе уровень логирования уже не поменять).
//
...
// Best-effort: if onnxruntime-web is used and already present, set its env logLevel too
try {
  const g: any = globalThis as any;
  if (g && g.ort && g.ort.env && desiredLevel !== undefined) {
    const map: Record<number, string> = { 0: 'verbose', 1: 'info', 2: 'warning', 3: 'error', 4: 'fatal' };
